...
function ItemUsageModule.UseItemOnLocation(npc, item, loc)
...
ItemUsageModule.Use['item_power_treads'] = function(item, bot, mode, extra_range)
	...
	if P.IsRetreating(bot) == true and tread_stat ~= ATTRIBUTE_STRENGTH and bot:WasRecentlyDamagedByAnyHero(5.0) == true then
		return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
	elseif P.IsRetreating(bot) == false and ItemUsageModule.IsHPHealing(bot) == true and bot:GetHealth() < 0.95*bot:GetMaxHealth() and tread_stat == ATTRIBUTE_STRENGTH then
		return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';	
	elseif ItemUsageModule.IsHPHealing(bot) == false and bot:WasRecentlyDamagedByAnyHero(3.0) == false and P.IsRetreating(bot) == false then
		local enemies = bot:GetNearbyHeroes( 1200, true, BOT_MODE_NONE );
...
--item_wraith_pact
ItemUsageModule.Use['item_wraith_pact'] = function(item, bot, mode, extra_range)
	
	if P.IsInPhalanxTeamFight(bot) and not P.IsRetreating(bot) then
		return BOT_ACTION_DESIRE_ABSOLUTE, bot:GetLocation(), 'point';
	end
